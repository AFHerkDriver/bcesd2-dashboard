...
# Source of truth: the department's own hydrants.json (SAWS master 2026-01-30 + A911 private
# plugs, via the PR Div Chief) — the same file both maps and both live tender flags read.
HYDB = "/home/user/bcesd2-dashboard/hydrants.json"
FT1000 = 304.8
SQMI = 2589988.0
ACRE = 4046.86
...
_hydb = json.load(open(HYDB))
def fetch(_unused, bbox):
    w, s, e, n = bbox
    return [(h[3], h[2]) for h in _hydb["h"] if w <= h[3] <= e and s <= h[2] <= n]
...
for k in ("esd2", "esd6"):
    ...
    report.append({"district": k, "hydrants_in_frame": len(all_pts), "db_version": _hydb.get("v"),
